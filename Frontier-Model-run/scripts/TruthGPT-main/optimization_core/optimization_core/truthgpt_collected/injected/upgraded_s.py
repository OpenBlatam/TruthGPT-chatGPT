...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None, past_kv=None):
        ...
        B, T_full, H, D = k.shape
        q = q.transpose(1, 2)
        k = k.transpose(1, 2)
        v = v.transpose(1, 2)
        if mask is None and T > 1:
            mask = torch.triu(torch.full((T, T_full), float('-inf'), device=x.device), diagonal=T_full - T + 1)
        ...
        return self.wo(attn_output), (k.transpose(1,2).contiguous() if T_full > 0 else None, v.transpose(1,2).contiguous() if T_full > 0 else None)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, mask=None, past_kv=None):
        norm_x = self.norm1(x)
        attn_out, new_kv = self.attention(norm_x, mask, past_kv)
        x = x + attn_out
...
class S(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, past_kv=None):
        ...
        for idx, layer in enumerate(self.layers):
            ...
            x, layer_new_kv = layer(x, attention_mask, layer_past)
            new_kv.append(layer_new_kv)
        x = self.norm(x)
        logits = self.output(x)
        return logits, new_kv

def get_model():
    model = S(vocab_size=32000, dim=512, num_layers=12, num_heads=8, max_seq_len=2048)
