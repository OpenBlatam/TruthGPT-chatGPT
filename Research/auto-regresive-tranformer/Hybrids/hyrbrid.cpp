...
int main() {
    ...
    model.to(torch::kHalf);  // Convert model to half-precision floating point

    // Initialize optimizer
    torch::optim::Adam optimizer(model.parameters(), torch::optim::AdamOptions(learning_rate).set_dtype(torch::kHalf));
    ...
    for (int epoch = 1; epoch <= num_epochs; epoch++) {
        ...
        for (int i = 0; i < input_seqs.size(); i += batch_size) {
            ...
            // Convert input and output sequences to tensors and move to GPU
            std::vector<torch::Tensor> batch_input_tensors, batch_output_tensors;
            for (int j = 0; j < batch_size; j++) {
                batch_input_tensors.push_back(torch::tensor(batch_input_ids[j]).to(device).to(torch::kHalf));
                batch_output_tensors.push_back(torch::tensor(batch_output_ids[j]).to(device).to(torch::kHalf));
            }

            // Train on batch and update epoch loss and number of time steps
            std::vector<torch::Tensor> result = trainer.train_batch(batch_input_tensors, batch_output_tensors);
